...
uint8_t pn532_tgInitAsTarget(const uint8_t* command, const uint8_t len, const uint16_t timeout){

  int8_t status = writeCommand(command, len, 0, 0);
    if (status < 0) {
        return -1;
    }
 ...
 * Peer to Peer
 */
uint8_t pn532_tgInitAsTargetComm(uint16_t timeout)
{
...
uint16_t pn532_tgGetData(uint8_t *buf, uint8_t len)
{
    buf[0] = PN532_COMMAND_TGGETDATA;

    if (writeCommand(buf, 1, 0, 0)) {
        return -1;
    }
    ...
    if(buf[0] == 0x29){
      //DMSG("target has been released by its initiator\n");
      return -6;
    }
...
int pn532_tgSetData(const uint8_t *header, uint8_t hlen, const uint8_t *body, uint8_t blen)
{
...
uint16_t pn532_inRelease(const uint8_t relevantTarget){

    pn532_packetbuffer[0] = PN532_COMMAND_INRELEASE;
    pn532_packetbuffer[1] = relevantTarget;

    if (writeCommand(pn532_packetbuffer, 2, 0, 0)) {
        return 0;
    }
